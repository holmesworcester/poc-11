...
fn proof_plan_records_unproven_to_unsuffixed_migration_and_link_domain_theorem() {
    ...
    for required in [
        ...
        "remains `_unproven` because actual byte parser/encoder proof, exact projected-chain-entry `Vec<FactId>` proof, and end-to-end core/replay composition are still open",
        ...
        "the actual byte parser/encoder, exact projected chain-entry vector contents, and end-to-end engine/replay composition remain open",
...
fn link_project_status_records_review_findings() {
    ...
    for required in [
        ...
        "Close exact projected-chain-entry vector contents",
        "routing runtime `ids`",
